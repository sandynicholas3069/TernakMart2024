        ...
        <form action="{{ route('product.update', $product->id) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <select name="kategori_produk" id="kategori_produk" class="mt-1 block w-full p-2 border border-gray-700 bg-white rounded-md focus:ring focus:ring-gray-500" required>
                    <option value="daging segar" {{ $product->kategori_produk == 'daging segar' ? 'selected' : '' }}>Daging Segar</option>
                    <option value="daging olahan" {{ $product->kategori_produk == 'daging olahan' ? 'selected' : '' }}>Daging Olahan</option>
                </select>
            </div>
            <div class="mb-4">
                ...
                <select name="kategori_daging" id="kategori_daging" class="mt-1 block w-full p-2 border border-gray-700 bg-white rounded-md focus:ring focus:ring-gray-500" required>
                    <option value="daging merah" {{ $product->kategori_daging == 'daging merah' ? 'selected' : '' }}>Daging Merah</option>
                    <option value="daging putih" {{ $product->kategori_daging == 'daging putih' ? 'selected' : '' }}>Daging Putih</option>
                </select>
            </div>
            <div class="mb-4">
                <label for="nama_produk" class="block text-sm font-medium text-white">Nama Produk:</label>
                <input type="text" name="nama_produk" id="nama_produk" value="{{ $product->nama_produk }}" class="mt-1 block w-full p-2 border border-gray-700 bg-white rounded-md focus:ring focus:ring-gray-500" required>
            </div>
            <div class="mb-4">
                <label for="harga_produk" class="block text-sm font-medium text-white">Harga Produk:</label>
                <input type="number" name="harga_produk" id="harga_produk" value="{{ $product->harga_produk }}" class="mt-1 block w-full p-2 border border-gray-700 bg-white rounded-md focus:ring focus:ring-gray-500" required>
            </div>
            <div class="mb-4">
                <label for="jumlah_stok" class="block text-sm font-medium text-white">Jumlah Stok:</label>
                <input type="number" name="jumlah_stok" id="jumlah_stok" value="{{ $product->jumlah_stok }}" class="mt-1 block w-full p-2 border border-gray-700 bg-white rounded-md focus:ring focus:ring-gray-500" required>
            </div>
