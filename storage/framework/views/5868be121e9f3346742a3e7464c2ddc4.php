    ...
    <div class="container mx-auto p-4">
        ...
        <div class="min-w-full py-6">
            <div class="bg-gray-800 shadow-lg rounded-lg p-6">
                <form method="GET" action="<?php echo e(route('product.performance')); ?>" class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <!-- Filter Area -->
                    <div>
                        <label for="day" class="block text-sm font-medium text-gray-400">Tanggal</label>
                        <input type="number" name="day" id="day" min="1" max="31" value="<?php echo e(request('day')); ?>" placeholder="Masukkan tanggal" class="mt-1 block w-full rounded-lg bg-gray-700 border border-gray-500 text-gray-200 px-4 py-2 focus:ring-gray-500 focus:border-gray-500">
                    </div>
                    <div>
                        <label for="month" class="block text-sm font-medium text-gray-400">Bulan</label>
                        <input type="number" name="month" id="month" min="1" max="12" value="<?php echo e(request('month')); ?>" placeholder="Masukkan bulan" class="mt-1 block w-full rounded-lg bg-gray-700 border border-gray-500 text-gray-200 px-4 py-2 focus:ring-gray-500 focus:border-gray-500">
                    </div>
                    <div>
                        <label for="year" class="block text-sm font-medium text-gray-400">Tahun</label>
                        <input type="number" name="year" id="year" min="2000" max="2100" value="<?php echo e(request('year')); ?>" placeholder="Masukkan tahun" class="mt-1 block w-full rounded-lg bg-gray-700 border border-gray-500 text-gray-200 px-4 py-2 focus:ring-gray-500 focus:border-gray-500">
                    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            var productPerformanceChart = new Chart(ctx, {
                ...
                data: {
                    labels: <?php echo json_encode($productSales->pluck('name')); ?>, // Product names
                    datasets: [{
                        label: 'Terjual',
                        data: <?php echo json_encode($productSales->pluck('total_sales')); ?>, // Sales data
                        backgroundColor: [
                            'rgba(255, 99, 132, 0.2)',
                            'rgba(54, 162, 235, 0.2)',
                            'rgba(255, 206, 86, 0.2)',
...
</html><?php /**PATH D:\Apps\laragon\www\ternakmart\resources\views/product_performance.blade.php ENDPATH**/ ?>